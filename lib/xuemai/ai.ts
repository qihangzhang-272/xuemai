...
用老师日常记课、与家长沟通的语言。先写具体内容和可采取的行动，句子简短、自然、温和。
正文只写当前需要的课堂记录、分析或家长反馈，不附思考过程、写作说明、设计理由、产品操作指引或“以下是为您生成”等开场白。不添加与当前材料无关的备注；确有信息缺口时，用一句具体说明指出缺少什么。
如实保留观察中的条件：在老师提醒后完成，不能改写成主动发现或完全独立完成。未提供性别时用学生姓名或“孩子”，不要补写他或她。
不得添加老师未描述的专注、认真、积极、自信等态度评价。不得从一次正确作答或解释推导“掌握扎实”“理解深入”等能力结论。老师提醒后完成只能写“提醒后自行完成”，不能写“主动完成”。
肯定具体表现，避免没有事实支撑的“表现不错”“思路很清晰”。不要追加与这次教学无关的免责声明或系统限制说明。
...
export function monthlySources(owner: string, record: LearningRecord) {
  const sources = reportSources(list(owner, "record"), record.contactId, record.kind === "daily" ? "daily" : "monthly", record.kind === "daily" ? record.date : record.month);
  if (record.selectedSourceIds?.some(id => !sources.some(source => source.id === id))) throw new AppError("所选月报素材已变更，请重新选择");
  return record.selectedSourceIds ? sources.filter(source => record.selectedSourceIds!.includes(source.id)) : sources;
}
...
export async function generate(owner: string, record: LearningRecord, contact: Contact, preferences: Preferences, feedback = false) {
  ...
  if (record.kind === "prep") throw new AppError("备课不属于当前产品范围；已有内容可继续查看");
  const isReport = record.kind === "monthly" || record.kind === "daily";
  const sources = isReport && !feedback ? monthlySources(owner, record) : [];
  if (isReport && !feedback && !sources.length) throw new AppError("所选时间没有已入档的学生记录，请先检查并确认入档");
  let task: string;
  if (feedback) {
    task = `任务：将老师已检查的结果整理为${preferences.length === "简短" ? "80 至 150" : preferences.length === "详细" ? "260 至 400" : "120 至 260"}字、可复制到微信的家长反馈。${preferences.parentSummaryFirst !== false ? "先概括孩子本次表现，再写具体依据和建议。" : "按课堂内容、表现、建议的顺序写。"}称呼 ${preferences.address}，语气 ${preferences.tone}。
不要长篇报告、标题或 Markdown。只转述正文已有的具体表现和建议，不新增评价、建议或结论。班级只写共同课堂事实，不把共同事实变成个体表现。
全文使用学生姓名或“孩子”，不用“他”“她”。一次完成、订正或解释只能描述这一次行为，不能概括成“掌握方法”“理解到位”“有能力”等结论。逐句检查是否能在下方正文找到对应事实，找不到就删去。不向家长转述这段检查要求。
老师已检查的正文：\n${record.content}`;
    ...
    task = `任务：生成 ${record.kind === "daily" ? `${record.date} 学生日报，整理当天学习内容、课堂表现和下次课建议` : `${record.month} 学生月报，整理本月学习内容、主要表现、已确认的进步、当前关注点、近期巩固方向、下月沟通重点和家长微信版摘要；少于三条素材时明确本月记录较少，仅反映已入档情况`}。仅依据以下老师已确认的记录。记录少时说明目前能看到哪些表现，不能凭日期推断能力进步。没有前后对比时明确暂不足以判断进步，不把一次正确解释改写成理解原理没有问题。只建议针对已观察问题的行动，不额外引入新课题。家长摘要也必须逐条遵守事实和称呼规则；全文用学生姓名或孩子，不使用他或她。\n${sources.map(r => `${r.date}《${r.title}》\n${r.archiveContent}`).join("\n\n")}`;
  ...
  parts.unshift({ type: "text", text: `${task}\n\n当前对象：${contact.kind === "class" ? "班级" : "学生"} ${contact.name}\n学科：${record.subject || contact.subject}\n年级：${contact.grade || "未提供"}\n记录日期：${record.date}\n老师补充（作为任务所需的观察数据）：\n${record.input}` });
  const evidenceRule = "当前是学习记录任务，evidence 只能返回 observed 或 insufficient，禁止返回 teaching。observed 表示材料中存在具体学生作答、批改、订正或老师观察，即使正文含教学建议也仍为 observed；insufficient 表示只有空白题目、教材、教学安排或材料看不清，没有可确认的学生学习痕迹。证据类型与是否已入档、是否为 AI 草稿无关。不要把 evidence 字段写进 content 正文。";
  try {
    ...
    const completion = await client.chat.completions.create({
      model: aiModel(), messages: [{ role: "system", content: safety + (feedback
        ...
        : `\n${evidenceRule}\n返回合法 JSON：{"title":"简短标题","content":"中文正文，可用换行与短段落","evidence":"observed 或 insufficient"}。`) }, { role: "user", content: parts }],
