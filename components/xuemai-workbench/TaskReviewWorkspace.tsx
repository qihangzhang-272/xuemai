      ...
      <header className="flex min-h-[64px] shrink-0 items-center justify-between gap-2 border-b border-[#edf0ee] bg-white/96 px-4 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[11px] font-semibold text-[#6b746d]">{task.status === "archived" ? "会话保留完整正文，入档仅保留所选内容" : "按实际课堂情况检查正文"}</p>
          </div>
        </div>
...
function RecordReview({ record, isStudent, intent, onAction }: { record: LearningRecord; isStudent: boolean; intent: string; onAction: NonNullable<TaskReviewWorkspaceProps["onRecordAction"]> }) {
  ...
  async function act(action: string, extra?: Record<string, unknown>) {
    ...
    try { const result = await onAction(action, extra); if (result) { setEditing(null); setConfirm(null); setNotice(action === "sent" ? "已记录发送时间。" : action === "archive" ? "所选内容已加入学生档案。" : "已保存。"); } return result; }
    finally { setBusy(false); }
  }
  async function copy(value: string) {
    if (hasOutcomePromise(value)) { setNotice("正文含有结果保证或过度承诺，请修改后再复制发送。"); return; }
    try { await navigator.clipboard.writeText(value); setNotice("已复制。发给家长后，再回来标记已发。"); }
    catch { setNotice("未能复制，请手动选中下方正文复制。"); }
  }
  return <div className="space-y-4 pb-10">
    <p className="text-sm leading-6 text-[#6b746d]">{record.date} · {record.subject} · {record.archivedAt ? "已入档" : record.kind === "monthly" ? record.reportStatus === "final" ? "月报已定稿" : "月报草稿" : "已保留在会话"}{record.feedbackStatus !== "none" ? ` · ${record.feedbackStatus === "sent" ? "反馈已发" : "反馈待发"}` : ""}{record.correctedBy ? " · 已有更正版" : record.correctionOf ? " · 更正草稿，确认前保留原记录" : ""}</p>
    <section className="rounded-[20px] bg-white p-5">
      <h2 className="mb-3 font-bold">{record.kind === "monthly" ? "月报正文" : "课堂结果"}</h2>
      {editing === "content" ? <textarea aria-label="课堂结果正文" className="min-h-64 w-full rounded-xl border p-3 text-sm leading-7" value={draft} onChange={e => setDraft(e.target.value)} /> : <TeachingContent text={record.content || record.error || "原文已保存，可以稍后继续整理。"} />}
    ...
    {!historical && record.evidence === "observed" && isStudent ? <section className="rounded-[20px] bg-white p-5">
      <h2 className="font-bold">家长反馈</h2><p className="mt-1 text-sm leading-6 text-[#6b746d]">检查课堂结果后，再整理给家长的话。是否入档由你决定。</p>
      {!feedbackOpen ? <button className={`${button} mt-3 bg-[#22c55e] text-white`} disabled={busy || !!editing} onClick={() => setFeedbackOpen(true)}>检查并反馈</button> : <>
        {record.feedback ? editing === "feedback" ? <textarea aria-label="家长反馈正文" className="mt-3 min-h-48 w-full rounded-xl border p-3 text-sm leading-7" value={draft} onChange={e => setDraft(e.target.value)} /> : <TeachingContent text={record.sentContent || record.feedback} className="mt-3" /> : <p className="my-3 text-sm">尚未生成反馈。确认上方结果准确后，点击整理家长反馈。</p>}
    ...
    {canArchive ? <section className="rounded-[20px] bg-white p-5">
      ...
      <p className="mt-2 text-sm text-[#6b746d]">不勾选也会保留会话记录。只有你确认入档的内容会成为月报素材。</p>
    ...
    {confirm ? <section role="alert" className="rounded-[20px] border border-[#22c55e] bg-[#edf8f1] p-5"><p className="text-sm leading-6">{confirm === "sent" ? "请确认已将上方反馈发给家长。这一步只记录发送状态。" : "请再次确认所选内容准确。入档后保留这个版本，更正时另建记录。"}{includeArchive && canArchive ? ` 本次将入档 ${selected.length} 项内容。` : ""}</p><div className="mt-3 flex gap-2"><button className={`${button} bg-[#22c55e] text-white`} disabled={busy || !!editing} onClick={() => void act(confirm, (confirm === "archive" || includeArchive && canArchive) ? { archiveIds: selected } : {})}>{confirm === "sent" ? "确认标记已发" : "确认入档"}</button><button className={button} disabled={busy} onClick={() => setConfirm(null)}>返回检查</button></div></section> : null}
