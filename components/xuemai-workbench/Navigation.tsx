...
export function ContactDirectory({ kind, contacts, onOpen, onProfile, onEdit, onCreate, onNavigation }: {
  ...
  return <div className="h-full overflow-y-auto bg-[#f6f7f6] p-4 sm:p-6"><div className="mx-auto max-w-[1000px] space-y-4">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><button onClick={onNavigation} className="mb-2 rounded-full bg-white px-3 py-2 text-sm lg:hidden">打开主导航</button><h1 className="text-[25px] font-bold">{label}</h1></div><button onClick={onCreate} className="rounded-full bg-[#22c55e] px-4 py-3 text-sm font-bold text-white">新建{label}</button></header>
